"""
Test the initial state used in the two-dimensional Bose-Hubbard
dynamics.
...
from dynamics_2d import BoseHubbardDynamics2D
...
def create_initial_state():
    ...
    dynamics = BoseHubbardDynamics2D(
        model,
        t_start=0,
        t_end=10,
        num_times=300,
...
def test_initial_state_is_pure():
    ...
    rho = ket2dm(psi0).to("csr") # It would be better to get the CSR format of the psi0, as psi0 could be in a dense format depending of the choice of state [whetherit is Fock state or normalized state!]
    # rho = ket2dm(psi0)
    purity = float(
        np.real_if_close(
            (rho * rho).tr()
